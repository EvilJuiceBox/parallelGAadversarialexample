...
def main(model):
    ...
    original = x_train[num]
    groundtruth = y_train[num]
    ...
    img.show()
    img.save("./exp/results/" + "original.png")

    collection = []
    for i in range(5):
        result = generateImageGA(populationSize=45, generation=1000, inputImage=original, model=model,
    ...
    collection = []
    for i in range(5):
        result = parallelGAhighConf(populationSize=15, generation=1000, inputImage=original, model=model,
                                    y_truth=np.argmax(groundtruth), IMAGEDIMENSION=32)
        temp = AdversarialExample(result)
        temp.setMSE(compare(original, result, 32))
        temp.setl1(getl1normdiff(original, result, 32))
