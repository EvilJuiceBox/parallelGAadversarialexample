...
def survivorSelection(original, inputPopulation, cullsize, elitism, model, y):
    ...
    for i in range(cullsize - elitism):  # pick the remaining survivors
        survivor, survivor1 = tournamentSelection(original, population, 3, model, y)
        temp.append(survivor)
        population = removeIndividual(population, survivor)  # prevent same individual from appearing twice

    temp = calculatePopulationFitness(original, temp, model, y)
    ...
    for i in range(generation):
        for z in range(islandCount):
            ...
            for j in range(numberOfChildren):
                offspring1, offspring2 = tournamentSelection(inputImage, islands[z], tournamentSize, model, y_truth)
                # Crossover operation
            ...
            tempPopulation = calculatePopulationHighConfFitness(inputImage, tempPopulation, model, y_truth)
            ...
            islands[z] = survivorSelection(inputImage, tempPopulation, populationSize, 3, model,
...
def parallelGA(populationSize, generation, inputImage, model, y_truth, IMAGEDIMENSION):
    ...
    for i in range(generation):
        for z in range(islandCount):
            ...
            # cull population down to original size, and proceed to next gen.
            tempPopulation = calculatePopulationFitness(inputImage, tempPopulation, model, y_truth)
            tempPopulation.sort(key=attrgetter("fitness"), reverse=False)
...
def generateImageGA(populationSize, generation, inputImage, model, y_truth, IMAGEDIMENSION):
    ...
    for i in range(generation):
        ...
        for j in range(numberOfChildren):
            offspring1, offspring2 = tournamentSelection(inputImage, population, tournamentSize, model, y_truth)
            # Crossover operation
            child1, child2 = crossover(offspring1, offspring2, IMAGEDIMENSION)
        ...
        population = survivorSelection(inputImage, tempPopulation, populationSize, 3, model,
                                       y_truth)  # elitism of 3 per round, chosen arbitrary
        if i % 100 == 0:
            print("End of generation: " + str(i) + "; Best performing member: " + str(
...
def tournamentSelection(inputImage, inp, tournamentSize, model,
    ...
    individuals = calculatePopulationFitness(inputImage, individuals, model, truth)
...
def getFitnessOnConstraint(original, image, model, truth, IMAGEDIMENSION):
    ...
    # L1 norm is number of pixel changes, range: (0, 32)  0-15 based on experiment
    # L2 norm is the sum of difference of pixel value squared    0.2
    # if random.randint(0, 100) == 1:
    #     print(str(y[0][np.argmax(y)]) + "L1 norm: " + str(getl1normdiff(original, image, IMAGEDIMENSION)) + "L2 norm: " + str(compare(original, image, IMAGEDIMENSION)))
...
#   returns -1 if the image does not match ground truth
#   returns the confidence interval of the image otherwise
def getHighConfFitness(original, image, model, truth, IMAGEDIMENSION):
    # image = candidateInput.getImage()
    x = np.expand_dims(image, 0)
    y = model.predict(x)
    lambda1 = 3
...
def calculatePopulationHighConfFitness(original, population, model, truth):
    temp = []
    for sample in population:
        item = Candidate(sample.getImage())
        item.setFitness(getHighConfFitness(original, sample.getImage(), model, truth, 32))
        temp.append(item)
    return temp


def calculatePopulationFitness(original, population, model, truth):
    temp = []
    for sample in population:
        item = Candidate(sample.getImage())
        item.setFitness(getFitnessOnConstraint(original, sample.getImage(), model, truth, IMAGEDIMENSION=32))
        temp.append(item)
    return temp
