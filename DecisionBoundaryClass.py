...
def calculate_distance_for_class(filepath, resultfolderpath, class_label=0, number_of_examples=3, count=10):
    ...
    while current_count <= count:
        ...
        if class_label == np.argmax(predicted) and np.argmax(ground_truth) == np.argmax(predicted): # if the image is of the correct prediction
            current_count += 1
            print(f"Generating adversarial example for predicted label {class_labels[np.argmax(predicted)]}")
            for i in range(num_classes):
                # if i == np.where(ground_truth == 1)[0][0]:
                #skip if the label is correct
                if i == np.argmax(ground_truth):
                    continue
        ...
        image_index += 1  # Always go to the next image

    saveResults(results, resultfolderpath, "decision_boundary_estimation")  # dumps the result, overwrites each old file.

    print(f"The total runtime is: {time.time() - runStartTime}")
...
def main(args):
    ...
    calculate_distance_for_class("resnet20.h5", "calculate_distance_class", class_label=8, number_of_examples=1)
