...
"""
def generateGaussianBlurResults(filepath, resultfolderpath, skip=0, count=30):
    ...
    while total < count:
        ...
        if groundtruth == basemodelprediction:
            ...
            reconstructedImg = DeepNeuralNetworkUtil.evenImage(pgaExample, beta=0.1)
...
def testEvenonSampleData(filepath, skip=0, count=1000):
    singleModel = load_model("./models/" + filepath)
    singleModel.summary()
    model = KumarDNN(singleModel)
    ...
    while total < count:
        ...
        if groundtruth == basemodelprediction:
            ...
            reconstructedImg = DeepNeuralNetworkUtil.evenImage(image, beta=0.1)
...
def main(args):
    # generateGaussianBlurResults("kumardnn.h5", "SVD test", count=30)
    testEvenonSampleData("kumardnn.h5", count=1000)
    ## 968 /1000 correct prediction on beta = 0.1
    ## 991 / 1000 correct predictions on beta = 0.05
    ...
    #
    # leena = DeepNeuralNetworkUtil.loadImg("leena.png", IMAGEDIMENSION=32)
    # blurred = DeepNeuralNetworkUtil.evenImage(leena, beta=0.1)
    # DeepNeuralNetworkUtil.show(blurred, IMAGEDIMENSION=32, standardised=False)
