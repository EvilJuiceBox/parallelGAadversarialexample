...
def main(args):
    ...
    with open(r"enkiresults.pickle", "rb") as input_file:
        enki_solutions = cPickle.load(input_file)

    with open(r"enkiresultslabel.pickle", "rb") as input_file:
        enki_label = cPickle.load(input_file)

    for item, label in zip(enki_solutions, enki_label):
        print(f"Enki: {label}")
        item = np.expand_dims(item, axis=0)
        y = model2.predict(item)
        print(f'shape: {item.shape}')
        print(f'Prediction: {y}')
        print(f'Prediction: {np.argmax(y)}')
        print(f'Prediction: {class_labels[np.argmax(y)]}')
        print(" ")
        # img = Image.fromarray(np.uint8(item.reshape(input_shape) * 255), 'RGB').resize((128, 128))
        # img.show()
