...
def main(model):
    ...
    collection = []
    for i in range(1):  ## changed to 1 for simple script
    ...
    file_path = 'cifar10_dnn.h5'
    print(f'Loading {file_path}...')
    modelx = load_model(file_path)
    modelx.summary()

    import _pickle as cPickle
    for i in range(len(collection)):
        print(np.uint8(collection[i].getImage().reshape(input_shape) * 255))
        ...
        saveimg = Image.fromarray(np.uint8(collection[i].getImage().reshape(input_shape)), 'RGB')
        print(saveimg)

        with open(r"adv.pickle", "wb") as output_file:
            cPickle.dump(collection[i].getImage().reshape(input_shape), output_file)

        saveimg.save("./exp/results/" + str(imgTitle) + ".png")
        ...
        print(f'Prediction: {np.argmax(y2)}')  # ground turth
        print(f'Prediction: {y[0][np.argmax(y2)]}')
        print("\n")
