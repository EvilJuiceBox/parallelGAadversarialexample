...
def main(args):
    ...
    for j in range(43, 45):
        print(f"----------- Starting experiment for {j} individuals ------------------")
        starting_index = 1000
        ...
        while total < 10:
            ...
            if groundtruth == basemodelprediction:
                ...
                resnetExample, resnetGeneration = AdvGenerator.generateImageGA(populationSize=j, generation=1000, inputImage=image, model=model,
                ...
                genTotal += resnetGeneration
                timeTotal += resnetEndTime
        gencoll.append(genTotal / 10)
        timecoll.append(timeTotal / 10)

    for x, y, z in zip(list(range(4, 15)), gencoll, timecoll):
        print(f"For GA run with population size {x}, the avg generation is {y}, and the avg time is {z}")
