...
def main(args):
    ensembleModel = DifferentEnsemble()
    ...
    while total < 30:
        ...
        if groundtruth == basemodelprediction and groundtruth == ensemblemodelprediction:
            # img = inverseStandardisation(image)
            # # printing results for resnet
            ...
            print(f"\n\tSingleModelTime: {resnetEndTime}")
            print(f"\tEnsembleModelTime: {ensembleEndtime}\n")

            if resnetGeneration >= 1000 or ensembleGeneration >= 1000:  # skips the last loop if over 1000 generations (fail to create one)
                total -= 1
                continue
            ...
            imgTitle = "./hetero3_09062021/" + str(imgTitle) + str(
            ...
            ensembleCollection.append(Result("Ensemble", ensembleGeneration, l1n, l2n, ensembleImage, failedpred, ensembleEndtime, image, starting_index + i))
            print("\n")
            for m, e in zip(collection, ensembleCollection):
                compareprint(m, e)
    ...
    pickle_out = open("hetero3_09062021/regularmodelresults.pickle", "wb")
    pickle.dump(collection, pickle_out)
    pickle_out.close()

    pickle_out = open("hetero3_09062021/ensemblemodelresult.pickle", "wb")
    pickle.dump(ensembleCollection, pickle_out)
    pickle_out.close()
